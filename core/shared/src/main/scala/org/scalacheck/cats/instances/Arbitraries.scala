...
object PartiallyAppliedFromOneAnd {
...
sealed private[instances] trait Arbitraries1 extends Arbitraries0 {
  ...
  def genEitherTWithErrorChannel[F[_], A : Arbitrary, B : Arbitrary, C : Arbitrary](implicit F: ApplicativeError[F, C]): Gen[EitherT[F, A, B]] =
    arbitrary[Either[C, Either[A, B]]].map {
      case Left(c) => EitherT(c.raiseError[F, Either[A, B]])
      case Right(ab) => EitherT.fromEither[F](ab)
    }

  implicit def arbEitherTWithErrorChannel[F[_], A : Arbitrary, B : Arbitrary, C : Arbitrary](implicit F: ApplicativeError[F, C]): Arbitrary[EitherT[F, A, B]] = Arbitrary(genEitherTWithErrorChannel[F, A, B, C])

  def genOptionTWithErrorChannel[F[_], A : Arbitrary, C : Arbitrary](implicit F: ApplicativeError[F, C]): Gen[OptionT[F, A]] =
